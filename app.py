...
with col2:
    ...
    document_title = st.text_input("Output Title", value="Course Outline")
...
if st.button("Generate Course Outline"):
    ...
    with st.spinner("Generating Learning Outcomes..."):
        ...
        learning_outcomes = generate_learning_outcomes(course_details=course_details, total_search_results=total_search_results, citation_style=citation_style)
    
    st.subheader("Learning Outcomes")
    st.write(learning_outcomes)

    with st.spinner("Generating Course Outline..."):
        # Generate Course Outline
        course_outline = generate_course_outline(course_details=course_details, learning_outcomes=learning_outcomes, number_of_topics=number_of_topics, total_hours=total_hours, weekly_hours=weekly_hours) 
...
if st.session_state.doc_path is not None and os.path.exists(st.session_state.doc_path):
    with open(st.session_state.doc_path, "rb") as file:
        btn = st.download_button(
            label="Download Course Outline",
            data=file,
            file_name=st.session_state.doc_path,
